...
namespace Modules.mikroElektronika
{
    /// <summary>
    ...
    /// </summary>
    public class LightningDetectorSPI
    {
        ...
        private byte TuneCapSetting = 6; //Based on tests with my detectors
        private readonly byte[] read2 = new byte[2];
        private readonly SPI spi;
        private readonly SPI.Configuration spiConfig;
        //default settings
        ...
        /// <summary></summary>
        /// <param name="calNumber">The socket that this module is plugged in to.</param>
        public LightningDetectorSPI(byte calNumber)
        {
            // from as3935 datasheet Chip Select (active low)
            // data are sampled on the falling edge of SCLK (CPHA=1)

            spiConfig = new SPI.Configuration(GHI.Pins.FEZCerbuinoBee.Socket1.Pin6, false, 0, 0, false, false, 1000, SPI.SPI_module.SPI1);

            spi = new SPI(spiConfig);

            TuneCapSetting = calNumber;
        ...
        {
            input = new InterruptPort(GHI.Pins.FEZCerbuinoBee.Socket1.Pin3, true, Port.ResistorMode.Disabled, Port.InterruptMode.InterruptEdgeHigh);
        ...
        public bool Calibrate()
        {
            ...
            spi.Write(wByte); //Calibration command
            Thread.Sleep(1);
            ...
            if (counter < 7)
                Debug.Print("Calibration Done  + count = " + counter);
            else
            {
                Debug.Print("Calibration Didn't Finish");
            }
            ...
            Debug.Print(success ? "Calibration Successful" : "Calibration failed");
        ...
        private void input_OnInterrupt(uint data1, uint data2, DateTime time)
        {
            Thread.Sleep(2);
            ...
            switch (source)
            {
                ...
                case 0x8: // Lightning
                    ...
                    break;
                default:
                    Debug.Print("Unknown interrupt source");
                    break;
            }
        }
        ...
        public void Reset()
        {
            ...
            Debug.Print("Tune Cap Setting " + RegisterRead(Register.TUN_CAP, BitMask.TUN_CAP));
